...
% TEST CASE III (Risk-averse) - from paper
beta_prime_init = [1; 1; 1];  % Risk-averse case (will show oscillations in old, smooth in new)
...
for k = 1:max_outer_iter
    ...
    % ====================================================================
    % STEP 3: DIRECT COMPUTATION OF f (NEW: closed-form, no consensus!)
    % From theory: f(alpha*) = (n + 4*sum(beta'))/(1 + n + 4*sum(beta'))
    % Using consensus term z_beta_sum(k)
    % ====================================================================
    f = (n_est + 4 * z_beta_sum) / denominator;
    history.f(k) = f;
