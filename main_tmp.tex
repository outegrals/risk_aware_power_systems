...
\begin{remark}[Scope of the Gaussian Assumption]
...
Since $\alpha\in[0,1]$, the parameter $\beta$ translates an operator's risk tolerance into a specific bid $\alpha$. This translation applies to both cooperative coalition games and noncooperative games, though the risk-neutral value of $\alpha$ differs between the two settings. 
...
\noindent  {\bf Corollary 5a} (Unconstrained ERS){\bf :} {\it In the general case of $n>2$ with equal risk-seeking (ERS), the optimal unconstrained bidding strategy is
\begin{equation}
...
\noindent {\bf Proof:} Follows directly from theorem~2 with $\beta_i = \beta_0 = (1-n)/(4a_2 n)$. \hfill $\Box$

\noindent  {\bf Corollary 5b} (Constrained ERS with overbidding){\bf :} {\it Under the ERS allocation of Corollary~5a, if overbidding is not permitted, operator $i$ overbids whenever $\eta_i < \frac{1}{2n}$. Define $\beta' = a_2\beta_0$ and $w = 1+4\beta'$ as the common scaled risk parameter. Let
\begin{equation} \Omega_s^P=\left\{j \in\Omega: \; \eta_j < \frac{w\left(1-\sum_{l\in\Omega_s^P}\eta_l\right)}{1+(n-|\Omega_s^P|)\, w}\right\}  \label{eq:overbid_bound}\end{equation}
be the ERS saturation set. [Note: $\Omega_s^P$ is distinct from the risk-neutral saturation set $\Omega_s$ of Corollary~4; the superscript $P$ reflects that operators saturate at different capacity thresholds under ERS versus risk-neutral strategies, since the saturation condition depends on $w = 1+4\beta'$.]
For the initial check with $\Omega_s^P=\varnothing$, condition~\eqref{eq:overbid_bound} reduces to $\eta_j < \frac{w}{1+nw}$, which at the ERS optimum $w=1/n$ gives $\eta_j < \frac{1}{2n}$.
Then the constrained ERS bidding strategy is
\begin{equation}
 \hspace*{-0.2in}    \alpha_i^* = \left\{ \begin{array}{ll} \displaystyle \frac{w}{\eta_i} \cdot \frac{1- \sum_{j\in\Omega_s^P} \eta_j}{1+(n-|\Omega_s^P|) w}  & i\not \in\Omega_s^P \\[6pt]
    1 & i\in\Omega_s^P \end{array} \right. \label{eq:alpha_clamp}
\end{equation}
and the total profit $\Pi$ in \eqref{eq:Pi_exp} is optimized with
\begin{eqnarray}
    g^{ERS}(\boldsymbol{\alpha}^*) & = & \frac{\left(\sum_{j\in\Omega_s^P}\eta_j + (n-|\Omega_s^P|)\, w\right)\left(1-\sum_{j\in\Omega_s^P}\eta_j\right)}{\left(1+(n-|\Omega_s^P|) w\right)^2} \nonumber \\
    && \times \; (x_r^o)^2.
...
\begin{equation}
\beta'^* = \frac{1}{4}\left(\frac{1-2\sum_{j\in\Omega_s^P}\eta_j}{n-|\Omega_s^P|} - 1 \right),
...
\noindent {\bf Proof:} For $i\not\in\Omega_s^P$, it follows from \eqref{eq:p1Ji=0} that, since $\beta_i = \beta$ for all $i$ (so $w_i = w$),
...
\begin{remark}[Communication Topology]
...
\section{Numerical Validation \label{sec:examples}}
