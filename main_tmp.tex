...
\noindent  {\bf Corollary 5:} {\it In the general case of $n>2$, the optimal equal-risk-seeking (ERS) biding strategy  is 
\begin{equation}
...
\noindent  {\bf Corollary 6:} {\it In the case that overbidding is not permitted under the Pareto-optimal risk-seeking strategy of theorem~2, the constrained optimal biding strategy is 
\begin{equation}
 \hspace*{-0.2in}    \bar{\alpha}_i = \left\{ \begin{array}{ll} \displaystyle \frac{w_i}{\eta_i} \cdot \frac{1- \sum_{j\in\Omega_s^P} \eta_j}{1+\sum_{j\not\in\Omega_s^P} w_j}  & i\not \in\Omega_s^P \\[6pt]
    1 & i\in\Omega_s^P \end{array} \right. \label{eq:alpha_clamp} 
\end{equation} 
where $w_i\triangleq 1+4a_2\beta_i$, $\Omega_s^P$ is the set of operators whose unconstrained bids \eqref{eq:alpha_optimal} exceed unity, and
\begin{equation} \Omega_s^P=\left\{j \in\Omega: \; a_2\beta_j > \tfrac{2\eta_j-1}{4}\right\}.  \label{eq:overbid_bound}\end{equation}
Furthermore, the constrained total profit is 
\begin{eqnarray}
\bar{\Pi} & = & 2a_2\left[\bar{f}^P(1-\bar{f}^P)(x_r^o)^2 - (x_r^o)^2 + x_{n+1}^ox_r^o \right] +a_1 x_r^o \nonumber \\
&& + \; 2a_2\left[\sum_{i=1}^n\sigma_{iL} - \sum_{i=1}^n\sum_{j=1}^n\sigma_{ij}\right], \label{eq:Pi_clamp}
\end{eqnarray}
where 
\begin{equation}
\bar{f}^{P}
= \sum_{j\in\Omega_s^P}\eta_j
+
\frac{\left(\sum_{j\not\in\Omega_s^P} w_j\right)\left(1-\sum_{j\in\Omega_s^P}\eta_j\right)}
{1+\sum_{j\not\in\Omega_s^P} w_j}.
\label{eq:fbar_P}
\end{equation}
}
 
\noindent {\bf Proof:} The proof parallels that of corollary~4. For $i\not\in\Omega_s^P$, it follows from \eqref{eq:p1Ji=0} that 
\[  w_i\sum_{j=1}^n(1-\bar{\alpha}_j)\eta_j = \bar{\alpha}_i \eta_i. \]
Since $\bar{\alpha}_j=1$ for $j\in\Omega_s^P$, the above equation becomes
\[ w_i\sum_{j\not\in\Omega_s^P} (1-\bar{\alpha}_j) \eta_j + \bar{\alpha}_i \eta_i= w_i\left(1- \sum_{j\in\Omega_s^P} \eta_j\right), \;\;\; \forall i\not\in\Omega_s^P.  \]
The solution to the above equation is given by \eqref{eq:alpha_clamp}. The expression for $\bar{f}^P$ in \eqref{eq:fbar_P} follows from direct substitution into \eqref{eq:F}, and $\bar{\Pi}$ in \eqref{eq:Pi_clamp} follows from \eqref{eq:Pi_exp}. \hfill $\Box$
 
Whenever $\Omega_s^P\neq\varnothing$, clamping pushes $\bar{f}^P$ below $\tfrac{1}{2}$ and reduces $g$ accordingly; this contrasts with the risk-neutral case of corollary~4, where clamping can move $f$ toward $\tfrac{1}{2}$. The sensitivity is illustrated numerically in Section~\ref{sec:examples}.
...
\textcolor{red}{Add discussions on distributed algorithm for non-overbidding}
