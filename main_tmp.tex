...
\begin{equation}
    \beta_i=\beta_0, \;\;\; \beta_0= \frac{1-n}{4a_2n}; \;\;\; \forall i\in\{1,\cdots,n\}. \label{eq:final_beta}
\end{equation}   
...
{\it Under ERS \eqref{eq:final_beta}, the common scaled risk parameter and
its associated weight are fixed by $\beta_0$ as
\begin{equation}
    \beta' = a_2\beta_0 = \frac{1-n}{4n}, \qquad w = 1+4\beta' = \frac{1}{n}.
    \label{eq:ERS_w}
\end{equation}
...
$\eta_i < \frac{1}{2n}$. With $w = \frac{1}{n}$, the ERS saturation set
\begin{equation}
    \Omega_s^P = \left\{j \in \Omega : \;
    \eta_j 
    \frac{1 - \sum_{l \in \Omega_s^P} \eta_l}
         {n\left(1 + \frac{n - |\Omega_s^P|}{n}\right)}
    \right\} \label{eq:overbid_bound}
\end{equation}
is distinct from the risk-neutral saturation set $\Omega_s$ of
Corollary~4, and reduces to $\eta_j < \frac{1}{2n}$ for the initial
check $\Omega_s^P = \varnothing$.
...
or equivalently,
\begin{equation}
    \alpha_i^* = \frac{n}{2n - |\Omega_s^P|} \cdot
    \frac{1 - \sum_{j \in \Omega_s^P}\eta_j}{\eta_i},
    \quad i \notin \Omega_s^P, \label{eq:alpha_clamp_simple}
\end{equation}
and the resulting objective value is
\begin{equation}
    g^{ERS}(\boldsymbol{\alpha}^*) =
    \frac{\left(\sum_{j \in \Omega_s^P}\!\eta_j
          + \frac{n - |\Omega_s^P|}{n}\right)
         \!\left(1 - \sum_{j \in \Omega_s^P}\!\eta_j\right)}
    {\left(1 + \frac{n - |\Omega_s^P|}{n}\right)^2}
    (x_r^o)^2. \label{eq:g_ERS_clamp}
\end{equation}
The optimal common risk parameter $\beta'^*$ maximizing
$g^{ERS}(\boldsymbol{\alpha}^*)$ is given by \eqref{eq:ERS_w} when
$\Omega_s^P = \varnothing$, and by
\begin{equation}
    \beta'^* = \frac{1}{4}\!\left(
    \frac{1 - 2\sum_{j \in \Omega_s^P}\eta_j}{n - |\Omega_s^P|}
    - 1\right) \label{eq:beta_opt_clamp}
\end{equation}
when $\Omega_s^P \neq \varnothing$, provided $\beta'^*$ does not enlarge
$\Omega_s^P$ beyond the assumed set; otherwise $\Omega_s^P$ and $\beta'^*$
must be determined jointly.

\noindent {\bf Proof:}
Part~(a) follows directly from Theorem~2 with $\beta_i = \beta_0 =
(1-n)/(4a_2 n)$, giving $w = 1/n$.

For part~(b), substituting $w = 1/n$ into \eqref{eq:p1Ji=0} with
$\alpha_j^* = 1$ for $j \in \Omega_s^P$ gives
\[
    \frac{1 - \sum_{j \in \Omega_s^P}\!\eta_j}{n}
    = \alpha_i^*\,\eta_i
    + \frac{1}{n}\!\sum_{j \notin \Omega_s^P}\!\alpha_j^*\,\eta_j,
    \quad \forall\, i \notin \Omega_s^P,
\]
whose solution is \eqref{eq:alpha_clamp_simple}. Substituting into
\eqref{eq:F} and \eqref{eq:G} yields \eqref{eq:g_ERS_clamp}.
Setting $f(\boldsymbol{\alpha}^*) = \tfrac{1}{2}$ and solving for the
required $w$ gives
\[
    w^* = \frac{1 - 2\sum_{j \in \Omega_s^P}\!\eta_j}{n - |\Omega_s^P|},
\]
from which \eqref{eq:beta_opt_clamp} follows via $\beta' = (w^*-1)/4$.
Since $w^* \neq 1/n$ whenever $\Omega_s^P \neq \varnothing$, the
constrained optimum requires adjusting $\beta'$ away from the ERS
value \eqref{eq:ERS_w}. \hfill $\Box$
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\linewidth]{figures/algorithm1_overbid_profit_individual.png}
    \caption{Individual profits $\Pi_i$ across three overbidding scenarios. Under unclamped overbidding, Op~3 gains at the expense of Ops~1 and~2; the no-overbidding projection restores a more balanced allocation while recovering near-optimal aggregate profit.}
    \label{fig:algo1_overbid_profit_individual}
\end{figure}
