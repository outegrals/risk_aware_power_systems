...
\begin{equation} 
\Delta \Pi = 2a_2 \left[ \sum_{i\in\Omega}\sum_{j\in\Omega}\sigma_{ij}- \sum_{i\in\Omega} \sigma_{iL}\right], \label{eq:D_P}
\end{equation}
...
\State \textbf{Phase 1.} Run average consensus~\eqref{eq:xi}--\eqref{eq:psi} until \mbox{$|\xi_i(k)-\xi_i(k-1)|\leq c$}:
\begin{equation}
...
\end{equation}
\State \textbf{Phase 2.} Compute individually $\hat{n}_i = 1/\xi_i - 1$, $\eta_i = x_i^o\xi_i/\psi_i$, and $\beta_i' \triangleq a_2\beta_i = (1-\hat{n}_i)/(4\hat{n}_i)$
\State \textbf{Phase 3.} Compute max/min consensus~$\eta_{\max}=\max_{i\in\Omega}\eta_i$, $\eta_{\min}=\min_{i\in\Omega}\eta_i$, and $\eta_c = (\eta_{\max}+\eta_{\min})/2$.
...
\Else 
    ...
    \If{$i\in \Omega_{\rm ERS}$}
        \State $\alpha_i^* = 1$
    \Else
        \State $\alpha_i^*=\dfrac{2\bigl(\sum_{j\not\in \Omega_{\rm ERS}}\eta_j\bigr)-1}{2\,(n-|\Omega_{\rm ERS}|)\,\eta_i}$
    \EndIf
\EndIf
\end{algorithmic}
\end{algorithm}
...
%The analysis assumes synchronous updates and lossless communication links. Robustness to asynchronous operation, link failures, and dynamic operator participation are important extensions beyond the scope of this paper and are noted as future work.
...
It follows from Lemma~1 that, if all operators cooperate and 
adopt the coalition strategy, their bids are $\alpha^*=\frac{1}{2}$ (with $\beta^*=0$), which yields $f(\boldsymbol{\alpha}^*)=\frac{1}{2}$, $g^C(\boldsymbol{\alpha}^*) = 22{,}500$\,MW$^2$, and the total profit of $\Pi^C = 10{,}800$\,\$/hr. This serves as the upper bound for the collective profit.

It follows from \eqref{eq:3p} with $\beta_i=0$ that the risk neutral Nash bidding strategies are $\boldsymbol{\alpha}^* =(\alpha_1^*, \alpha_2^*, \alpha_3^*) = (0.625, 0.75, 0.9375)$, which results in $f(\boldsymbol{\alpha}^*)=0.75$, $g(\boldsymbol{\alpha}^*)=16{,}875$\,MW$^2$, and the total profit $\Pi^{RN} = 9{,}675$\,\$/hr. In this case, the collective profit is $10.42\%$ below that under the coalition solution. On the other hand, risk seeking Pareto-optimal Nash bidding strategies in \eqref{eq:3p} allow different choices of risk seeking levels $\beta_i$ by different operators as long as constraint \eqref{eq:beta_sum_condition} is satisfied. Admissible choices of $\beta_i$ form in Figure~\ref{fig:beta_rs}. Should all renewable operators agree to choose the same risk tolerance level, we know from \eqref{eq:beta_sum_condition} that $\beta_i^* = -\frac{5}{3} $ (equivalently $\beta_i^{\prime *} = a_2\beta_i^* = -\frac{1}{6}$), which yields $\boldsymbol{\alpha}^*=(0.4167, 0.5000, 0.6250)$, resulting in $f(\boldsymbol{\alpha}^*)=0.5$, $g(\boldsymbol{\alpha}^*)=22{,}500$\,MW$^2$, and the total profit $\Pi^{RS} = 10{,}800$\,\$/hr. This risk seeking Pareto-optimal Nash solution fully recovers the total profit under the coalition strategy.
...
The partition~\eqref{eq:partition_Omega} gives $\eta_{\max}=0.48$, $\eta_{\min}=0.12$, midpoint $\eta_c=0.30$, hence $\Omega_{\rm ERS}=\{3\}$ and $\Omega\setminus\Omega_{\rm ERS}=\{1,2\}$. Substituting into~\eqref{eq:beta_hetero}--\eqref{eq:alpha_hetero} with $\sum_{j\not\in\Omega_{\rm ERS}}\eta_j=0.88$, $\sum_{j\in\Omega_{\rm ERS}}\eta_j=0.12$, $n-|\Omega_{\rm ERS}|=2$:
\begin{align}
...
The Corollary~5b branch of Algorithm~1 is validated on Scenarios~2 and~3.  In each case operators first run max- and min-consensus on $\eta_i$ to recover $\eta_{\max}$ and $\eta_{\min}$ (Phase~3a), then each operator locally computes $\eta_c=(\eta_{\max}+\eta_{\min})/2$ and determines its partition membership.  Operators not in $\Omega_{\rm ERS}$ then execute Phase~4: a second $\xi$-protocol seeded with $\xi_i^{(2)}(0)=1$ for $i\not\in\Omega_{\rm ERS}$ and $0$ otherwise, recovering $n-|\Omega_{\rm ERS}|=\xi_i^{(2),\infty}/\xi_i^\infty$; and a $\psi$-protocol seeded with $\psi_i^{(2)}(0)=\eta_i$ for $i\not\in\Omega_{\rm ERS}$ and $0$ otherwise, recovering $\sum_{j\not\in\Omega_{\rm ERS}}\eta_j=\psi_i^{(2),\infty}/\xi_i^\infty$.  In Scenario~2 ($\eta_c=0.30$): Ops~1 and~2 are not in $\Omega_{\rm ERS}$ and seed the Phase~4 protocols, while Op~3 ($\in\Omega_{\rm ERS}$) sets $\alpha_3^*=1$ directly.  In Scenario~3 ($\eta_c=0.425$): Op~1 alone is not in $\Omega_{\rm ERS}$ and executes Phase~4, while Ops~2 and~3 ($\in\Omega_{\rm ERS}$) saturate at $\alpha_{2,3}^*=1$.  In both cases the Phase~4 consensus rounds converge and the locally recovered $(n-|\Omega_{\rm ERS}|)$ and $\sum_{j\not\in\Omega_{\rm ERS}}\eta_j$ yield bids $\boldsymbol{\alpha}^*$ indistinguishable from the centralized Corollary~5b solution, confirming that partition information is correctly propagated without any central coordinator.
