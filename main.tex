...
\begin{equation}
 \hspace*{-0.2in}    \alpha_i^* = \left\{ \begin{array}{ll} \displaystyle \frac{1}{n+1-|\Omega_{\rm RN}|}\eta_i^{-1} \left[ 1- \sum_{j\in\Omega_{\rm RN}} \eta_j\right]  & i\not \in\Omega_{\rm RN} \\
    1 & i\in\Omega_{\rm RN} \end{array} \right. \label{eq:alpha_i_1} 
\end{equation} 
where $\Omega_{\rm RN}$ is the set of operators who are constrained to overbid, $|\Omega_{\rm RN}|$ denotes its cardinality, and
\[ \Omega_{\rm RN}=\left\{j \in\Omega: \; \eta_j<\tfrac{1}{n+1}\right\}.  \]
Furthermore, the total profit $\Pi$ in \eqref{eq:Pi_exp} is optimized with} 
\begin{eqnarray*}
    g^{RN}(\boldsymbol{\alpha}^*) &= & \frac{1}{(n+1-|\Omega_{\rm RN}|)^2} \left[ n-|\Omega_{\rm RN}| +\sum_{j\in\Omega_{\rm RN}} \eta_j \right] \\
    && \times \left[ 1 -\sum_{j\in\Omega_{\rm RN}} \eta_j \right](x_r^o)^2
\end{eqnarray*}

\noindent {\bf Proof:} For $i\not\in\Omega_{\rm RN}$, it follows from \eqref{eq:p1Ji=0} that, with $\beta_i=0$,
\begin{equation*}
  \sum_{j=1}^n(1-\alpha_j)\eta_j - \alpha_i \eta_i=0.
\end{equation*} 
Since $\alpha_j=1$ for $j\in\Omega_{\rm RN}$, the above equation becomes
\[ \sum_{j\not\in\Omega_{\rm RN}} \alpha_j \eta_j + \alpha_i \eta_i= 1- \sum_{j\in\Omega_{\rm RN}} \eta_j, \;\;\; \forall i\not\in\Omega_{\rm RN}.  \]
...
& = & \frac{n-|\Omega_{\rm RN}|}{n+1-|\Omega_{\rm RN}|} +\frac{1}{n+1-|\Omega_{\rm RN}|}\sum_{j\in\Omega_{\rm RN}} \eta_j,
\label{eq:F_1}
\end{eqnarray*} 
and
\[ g(\boldsymbol{\alpha}^*) = f(\boldsymbol{\alpha}^*) \left[ 1 - f(\boldsymbol{\alpha}^*)  \right](x_r^o)^2, \]
...
{\it Under ERS \eqref{eq:final_beta}, substituting $\beta_i = \beta_0$ into \eqref{eq:alpha_optimal} gives
...
\noindent{\bf (b) Constrained ERS with fixed $\beta_0$ (clamped bids).}
If overbidding is not permitted and operators retain $\beta_i=\beta_0$,
let
\begin{equation}
    \Omega_{\rm ERS} = \Bigl\{j \in \Omega : \eta_j < \tfrac{1}{2n}\Bigr\}
    \label{eq:overbid_bound}
\end{equation}
denote the ERS saturation set with $|\Omega_{\rm ERS}|$ its cardinality.
The constrained bidding strategy is
\begin{equation}
    \alpha_i^* = \begin{cases}
        \displaystyle
        \frac{n\!\left(1-S\right)}
             {(2n-|\Omega_{\rm ERS}|)\,\eta_i}
        & i \notin \Omega_{\rm ERS}, \\[10pt]
        1 & i \in \Omega_{\rm ERS},
    \end{cases} \label{eq:alpha_clamp}
\end{equation}
and the total profit $\Pi$ in \eqref{eq:Pi_exp} satisfies
\begin{equation}
    g^{ERS}(\boldsymbol{\alpha}^*) =
    \frac{\bigl(S(2n-|\Omega_{\rm ERS}|) + n - |\Omega_{\rm ERS}|\bigr)
          (1-S)}
         {(2n-|\Omega_{\rm ERS}|)^2}
    (x_r^o)^2, \label{eq:g_ERS_clamp}
\end{equation}
where $S = \sum_{j\in\Omega_{\rm ERS}}\eta_j$,
with $f(\boldsymbol{\alpha}^*) \leq \tfrac{1}{2}$, so
$g^{ERS}(\boldsymbol{\alpha}^*) \leq g^{\max}$ in general.}

\noindent {\bf Proof:}

Part~(a) follows from Theorem~2 with $\beta_i = \beta_0 = (1-n)/(4a_2n)$
for all $i$. Substituting into \eqref{eq:alpha_optimal} gives
$1 + 4a_2\beta_0 = 1/n$, and hence $\alpha_i^* = \frac{1}{2n}\eta_i^{-1}$.
It follows that $f(\boldsymbol{\alpha}^*) = \frac{1}{2n}\sum_i 1 = \frac{1}{2}$,
confirming $g^{ERS}(\boldsymbol{\alpha}^*) = \frac{1}{4}(x_r^o)^2 = g^{\max}$
via \eqref{eq:G}.

For part~(b), it follows from \eqref{eq:p1Ji=0} that, with
$\beta_i = \beta_0$ for all $i$,
$\frac{1}{n}\sum_{j=1}^n(1-\alpha_j^*)\eta_j = \alpha_i^*\eta_i$.
Since $\alpha_j^* = 1$ for $j \in \Omega_{\rm ERS}$, this becomes
\[
    \frac{1}{n}\sum_{j \notin \Omega_{\rm ERS}}\alpha_j^*\eta_j + \alpha_i^*\eta_i
    = \frac{1-S}{n},
    \quad \forall\, i \notin \Omega_{\rm ERS}.
\]
Solving these equations yields \eqref{eq:alpha_clamp}. It then
follows from \eqref{eq:F} that
\[
    f(\boldsymbol{\alpha}^*) =
    \frac{n-|\Omega_{\rm ERS}|}{2n-|\Omega_{\rm ERS}|}
    (1-S)
    + S,
\]
and $g^{ERS}(\boldsymbol{\alpha}^*)=f(\boldsymbol{\alpha}^*)[1-f(\boldsymbol{\alpha}^*)](x_r^o)^2$,
which gives \eqref{eq:g_ERS_clamp}. \hfill $\Box$

\noindent\textbf{Remark~1.}
\textit{When the no-overbidding constraint is active but the Pareto condition
$f(\boldsymbol{\alpha}^*)=\tfrac{1}{2}$ is additionally required,
the common risk parameter must be re-optimized to
\begin{equation}
    \beta'^* = \frac{1}{4}\!\left(
    \frac{1 - 2S}{n - |\Omega_{\rm ERS}|}
    - 1\right), \label{eq:beta_opt_clamp}
\end{equation}
where $S=\sum_{j\in\Omega_{\rm ERS}}\eta_j$,
valid provided the resulting $\boldsymbol{\alpha}^*$ does not expand
$\Omega_{\rm ERS}$ beyond the set assumed in its construction.
This restores $g^{ERS}(\boldsymbol{\alpha}^*)=g^{\max}$ with no efficiency
penalty, and is the case illustrated in Scenario~2 of Section~VI-C.
The derivation follows by substituting \eqref{eq:alpha_clamp} into
$f(\boldsymbol{\alpha}^*)=\tfrac{1}{2}$ and solving for $\beta'^*$.}
...
When the no-overbidding constraint is active, the distributed implementation requires a minor modification. After consensus convergence, each operator locally checks whether its unconstrained bid $\alpha_i^* = \frac{1}{2n}\eta_i^{-1}$ exceeds unity, which occurs whenever $\eta_i < \frac{1}{2n}$. If so, operator $i$ projects its bid to $\bar{\alpha}_i = 1$ and belongs to the saturated set $\Omega_{\rm ERS}$ of Corollary~5b. The key observation is that the constrained bid~\eqref{eq:alpha_clamp} for unsaturated operators depends on the aggregate quantity $\sum_{j\in\Omega_{\rm ERS}}\eta_j$, which can be estimated through an additional consensus round. Specifically, after each operator determines its own saturation status locally, a third consensus protocol initialized with $\phi_i(0) = \eta_i$ if $i\in\Omega_{\rm ERS}$ and $\phi_i(0) = 0$ otherwise yields the average $\frac{1}{n+1}\sum_{j\in\Omega_{\rm ERS}}\eta_j$, from which each operator recovers $\sum_{j\in\Omega_{\rm ERS}}\eta_j$. Combined with the already-available $n$ and $\eta_i$, each unsaturated operator can then compute its constrained bid~\eqref{eq:alpha_clamp} without centralized coordination. If the Pareto condition is additionally required, $\beta'^*$ is computed via Remark~1 using the recovered $\sum_{j\in\Omega_{\rm ERS}}\eta_j$.
\begin{comment}
...
To illustrate the constrained branch of Corollary~5b, we introduce \textit{Scenario~2} by setting $x_3^o = 30$\,MW, giving $x_r^o = 250$\,MW and $\eta_3 = \frac{30}{250} = 0.12 < \frac{1}{6} = \frac{1}{2n}$. Operator~3 therefore belongs to $\Omega_{\rm ERS} = \{3\}$: its unconstrained bid at $\beta'=-\frac{1}{6}$ equals $\frac{1}{2n\eta_3} = \frac{1}{6\times 0.12}\approx 1.39 > 1$.

Table~\ref{tab:overbid_compare} compares two policies for Scenario~2. If overbidding is permitted, Op~3 bids $\alpha_3^*\approx1.39$ at $\beta'=-\frac{1}{6}$; the Pareto condition still holds ($f=0.5$, $g=g^{\max}$), so there is no efficiency loss, only a feasibility violation. Remark~1 provides the no-overbidding Pareto-restoring alternative: prescribing $\beta'^*$ via~\eqref{eq:beta_opt_clamp} so that the constraint binds exactly without any efficiency loss. With $|\Omega_{\rm ERS}|=1$ and $\sum_{j\in\Omega_{\rm ERS}}\eta_j = 0.12$, solving $f(\boldsymbol{\alpha}^*)=\frac{1}{2}$ gives
\[
  w^* = \frac{1 - 2\eta_3}{n - |\Omega_{\rm ERS}|} = \frac{1 - 0.24}{2} = 0.38,
  \qquad
...
\hline
Scenario & $\Omega_{\rm ERS}$ & $\beta'^*$ & $\alpha_1^*$ & $\alpha_2^*$ & $\alpha_3^*$ & $f$ & $g$\,(MW$^2$) & $\sum\Pi_i$\,(\$/hr) & loss \\
\hline
...
\noindent\textbf{Remark~2.} \textit{Overbidding at the ERS optimum is not an artifact of deviating from $\beta'^*$, but a structural consequence of small renewable share: operator~$i$ overbids whenever $\eta_i < \frac{1}{2n}$, regardless of the chosen common $\beta'$. In such cases, the re-optimized $\beta'^*$ of Remark~1 reallocates bids so that the capacity-limited operator is fully dispatched ($\alpha_i^*=1$) while the remaining operators restore $f=\frac{1}{2}$, recovering $g^{\max}$ with no efficiency penalty.}
